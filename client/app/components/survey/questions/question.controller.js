class QuestionController {
  ...
  reject = () => {
    this.questionService.rejectAnswer(this.answer.id, this.$state.$current.no);
  }

  approve = () => {
    this.questionService.approveAnswer(this.answer.id, this.$state.$current.no);
  }
  ...
  getTotalDIPA = () => {
    return this.formatMoney((this.parseMoney(this.answer.data.dipa_danapemerintah) + this.parseMoney(this.getTotalPNPB()) + this.parseMoney(this.answer.data.dipa_phln)), 2, ',', '.');
  }

  getTotalNonDIPA = () => {
    return this.formatMoney((this.parseMoney(this.answer.data.nondipa_insentif_ristekdikti) + this.parseMoney(this.answer.data.nondipa_insentif_researchgrant) + this.parseMoney(this.answer.data.nondipa_insentif_dalamnegeri)), 2, ',', '.');
  }

  getTotalPNPB = () => {
    return this.formatMoney((this.parseMoney(this.answer.data.dipa_pnbp_perusahaanswasta) + this.parseMoney(this.answer.data.dipa_pnbp_instansipemerintah) + this.parseMoney(this.answer.data.dipa_pnbp_swastanonprofit) + this.parseMoney(this.answer.data.dipa_pnbp_luarnegeri)), 2, ',', '.');
  }

  getTotalQ4 = () => {
    return this.formatMoney((this.parseMoney(this.answer.data.belanja_pegawai_upah) + this.parseMoney(this.answer.data.belanja_modal_properti) + this.parseMoney(this.answer.data.belanja_modal_mesin) + this.parseMoney(this.answer.data.belanja_operasional_maintenance)), 2, ',', '.');
  }

  getTotalQ7 = () => {
    return this.formatMoney((this.parseMoney(this.answer.data.penelitian_dasar) + this.parseMoney(this.answer.data.penelitian_terapan) + this.parseMoney(this.answer.data.pengembangan_eksperimental)), 2, ',', '.');
  }
